...
The indicator function $[x_i > \Delta_{k-1}]$ evaluates to $1$ if true and $0$ if false [@knuth1992]. That is, for a given `x_i`, the parameters of the current and previous segments predict $\mu_i$ while future segments are multiplied by zero. The indicator can be ignored for the first segment ($k = 1$, corresponding to setting $\Delta_0 = -\infty$) or, as `mcp` defaults to, the change point can be restricted to the observed range of $\mathbf{x}$ by setting $\Delta_0 = \min(\mathbf{x})$. Also,
...
## Relative, absolute, and segment-specific models
...
<!-- How to formally separate modeled y from observed y? -->
